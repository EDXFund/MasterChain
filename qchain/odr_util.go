...
func GetBody(ctx context.Context, odr OdrBackend, shardId uint16,hash common.Hash, number uint64) (*types.SuperBody, error) {
	...
	bodyData := new(types.BodyEncode)
	body := new (types.SuperBody)
	if err := rlp.Decode(bytes.NewReader(data), bodyData); err != nil {
		rlp.DecodeBytes(bodyData.Body,body)
	}
	return body, nil
}
...
func GetBlock(ctx context.Context, odr OdrBackend, shardId uint16,hash common.Hash, number uint64) (types.BlockIntf, error) {
	...
	if shardId == types.ShardMaster {
		...
		return types.NewSBlockWithHeader(header).WithBody(body.ShardBlocks, body.Receipts,body.Transactions, body.Results), nil
