	...
	return func(n uint64) common.Hash {
		...
		if cache == nil {
			cache = map[uint64]common.Hash{
				ref.NumberU64() - 1: ref.ParentHash(),
			}
		}
		...
		for header := chain.GetHeader(ref.ParentHash(), ref.NumberU64()-1); header != nil; header = chain.GetHeader(header.ParentHash(), header.NumberU64()-1) {
			cache[header.NumberU64()-1] = header.ParentHash()
