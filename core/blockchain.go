...
// modified to fit EDX specifications by Tony Zhu
package core
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	delFn := func(db rawdb.DatabaseDeleter, hash common.Hash, num uint64) {
		rawdb.DeleteBody(db, bc.ShardId(), hash, num)
	}
	...
	if currentFastBlock := bc.CurrentFastBlock(); currentFastBlock != nil && currentHeader.NumberU64() < currentFastBlock.NumberU64() {
...
func (bc *BlockChain) CurrentBlock() types.BlockIntf {
	return bc.currentBlock.Load().(types.BlockIntf)
}

// CurrentFastBlock retrieves the current fast-sync head block of the canonical
// chain. The block is retrieved from the blockchain's internal cache.
func (bc *BlockChain) CurrentFastBlock() types.BlockIntf {
	return bc.currentFastBlock.Load().(types.BlockIntf)
}
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis types.BlockIntf) error {
	...
	rawdb.WriteBlock(bc.db, genesis.ToBlock())
...
func (bc *BlockChain) repair(head *types.BlockIntf) error {
	for {
...
func (bc *BlockChain) insert(block types.BlockIntf) {
	// If the block is on a side chain or an unknown one, force other heads onto it too
	updateHeads := rawdb.ReadCanonicalHash(bc.db, bc.ShardId(), block.NumberU64()) != block.Hash()

	// Add the block to the canonical chain number scheme and mark as the head
	rawdb.WriteCanonicalHash(bc.db, bc.ShardId(), block.Hash(), block.NumberU64())
	rawdb.WriteHeadBlockHash(bc.db, bc.ShardId(), block.Hash())
	...
	if updateHeads {
		...
		rawdb.WriteHeadFastBlockHash(bc.db, bc.ShardId(), block.Hash())
...
func (bc *BlockChain) GetUnclesInChain(block types.BlockIntf, length int) []types.HeaderIntf {
	...
	unclesIntf := make([]types.HeaderIntf,len(uncles))
...
func (bc *BlockChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if currentHeader.Hash() == hash {
			bc.hc.SetCurrentHeader(bc.GetHeader(currentHeader.ParentHash(), currentHeader.NumberU64()-1))
		}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.BlockIntfs, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		rawdb.WriteBody(batch, block.Hash(), bc.shardId, block.NumberU64(), block.Body())
...
///  4)当前是子链节点，收到了主链的区块，根据记录的主链区块信息，重置自身的链
	...
	if len(chain) > 0 {
		if bc.shardId == chain[0].ShardId() {
			...
			if bc.shardId == types.ShardMaster { //主链收到了子链区块的信息，只需插入到shardManager中去
				...
				if chain[0].ShardId() == types.ShardMaster {
					return bc.shardProcMasterBlock(chain)
				}
				// else 子链收到其它子链的区块，直接丢弃
				return 0, nil
			}
		}
	} else {
...
func (bc *BlockChain) insertChain(chain types.BlockIntfs) (int, []interface{}, []*types.Log, error) {
...
func (st *insertStats) report(chain []types.BlockIntf, index int, cache common.StorageSize) {
...
func countTransactions(chain []types.BlockIntf) (c int) {
	for _, b := range chain {
...
// event about them
func (bc *BlockChain) reorg(oldBlock, newBlock types.BlockIntf) error {
	fmt.Println(" old:", oldBlock.Hash(), " new:", newBlock.Hash())
	var (
		newChain          types.BlockIntfs
		oldChain          types.BlockIntfs
		commonBlock       types.BlockIntf
		deletedTxs        types.Transactions
		deletedShardInfos types.ShardBlockInfos
	...
	for {
		...
		deletedTxs = append(deletedTxs, oldBlock.Transactions()...)
		deletedShardInfos = append(deletedShardInfos, oldBlock.ShardBlocks()...)
		collectLogs(oldBlock.Hash())
	...
	if bc.ShardId() == types.ShardMaster {
		bc.reorgShardInfos(newChain, deletedShardInfos)
	} else {
		bc.reorgTxs(newChain, deletedTxs)
	}
...
func (bc *BlockChain) reorgTxs(newChain types.BlockIntfs, deletedTxs types.Transactions) {
...
func (bc *BlockChain) reorgShardInfos(newChain types.BlockIntfs, deletedShardBlocks types.ShardBlockInfos) {
	// Insert the new chain, taking care of the proper incremental order
	var addedSfs types.ShardBlockInfos
	for i := len(newChain) - 1; i >= 0; i-- {
		// insert the block in the canonical way, re-writing history
		bc.insert(newChain[i])
		// write lookup entries for hash based transaction/receipt searches
		rawdb.WriteShardBlockEntries(bc.db, newChain[i].ToBlock())
		addedSfs = append(addedSfs, newChain[i].ShardBlocks()...)
	}
	// calculate the difference between deleted and added transactions
	diff := types.ShardInfoDifference(deletedShardBlocks, addedSfs)
	// When transactions get deleted from the database that means the
	// receipts that were created in the fork must also be deleted
	batch := bc.db.NewBatch()
	for _, tx := range diff {
...
func (bc *BlockChain) BadBlocks() []types.BlockIntf {
	blocks := make([]types.BlockIntf, 0, bc.badBlocks.Len())
	for _, hash := range bc.badBlocks.Keys() {
		if blk, exist := bc.badBlocks.Peek(hash); exist {
			block := blk.(types.BlockIntf)
			blocks = append(blocks, block)
		}
	}
	return blocks
...
func (bc *BlockChain) addBadBlock(block types.BlockIntf) {
	bc.badBlocks.Add(block.Hash(), block)
}

// reportBlock logs a bad block error.
func (bc *BlockChain) reportBlock(block types.BlockIntf, receipts types.Receipts, err error) {
...
func (bc *BlockChain) writeHeader(header types.HeaderIntf) error {
	bc.wg.Add(1)
	defer bc.wg.Done()
