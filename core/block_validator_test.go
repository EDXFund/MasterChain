	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				_, results = engine.VerifyHeaders(chain, []types.HeaderIntf{headers[i]}, []bool{true})
			} else {
				engine := ethash.NewFakeFailer(headers[i].NumberU64())
				_, results = engine.VerifyHeaders(chain, []types.HeaderIntf{headers[i]}, []bool{true})
			}
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, ethash.NewFakeFailer(uint64(len(headers)-1)), vm.Config{}, nil,types.ShardMaster)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, ethash.NewFakeDelayer(time.Millisecond), vm.Config{}, nil,types.ShardMaster)
	defer chain.Stop()
