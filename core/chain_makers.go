...
type BlockGen struct {
	i           int
	parent      types.BlockIntf
	chain       []types.BlockIntf
...
func (b *BlockGen) AddTxWithChain(bc *BlockChain, tx *types.Transaction) {
	...
	receipt, _, err := ApplyTransaction(b.config, bc, b.header.CoinbasePtr(), b.gasPool, b.statedb, b.header, tx, b.header.GasUsedPtr(), vm.Config{})
	if err != nil {
		panic(err)
	}
...
func (b *BlockGen) OffsetTime(seconds int64) {
	//fmt.Println("--- step1.6 of",b.header.Number().Uint64(),"time:",b.header.Time().Uint64(),"diff:",b.header.Difficulty())

	b.header.SetTime(b.header.Time().Add(b.header.Time(), new(big.Int).SetInt64(seconds)))
...
func GenerateChain(config *params.ChainConfig, parent types.BlockIntf, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) ([]types.BlockIntf, []types.Receipts) {
	...
	blocks, receipts := make(types.BlockIntfs, n), make([]types.Receipts, n)
...
func makeHeader(chain consensus.ChainReader, parent types.BlockIntf, state *state.StateDB, engine consensus.Engine) types.HeaderIntf {
	var time *big.Int
	if parent.Time() == nil {
		time = big.NewInt(10)
	} else {
...
func makeBlockChain(parent types.BlockIntf, n int, engine consensus.Engine, db ethdb.Database, seed int) []types.BlockIntf {
