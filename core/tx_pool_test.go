...
func (bc *testBlockChain) CurrentBlock() types.BlockIntf {
	if bc.shardId == types.ShardMaster  {
		...
		return types.NewSBlock(header, nil)
	}
...
func (bc *testBlockChain) GetShardBlock(shardId uint16, hash  common.Hash,number uint64) (types.BlockIntf) {
	return nil
}
...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil, 0), types.HomesteadSigner{}, key)
	return tx
}
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil, 0), types.HomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func testTransactionDoubleNonce(t *testing.T,shardId uint16) {
	...
	tx1, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil, 0), signer, key)
	tx2, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil, 0), signer, key)
	tx3, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil, 0), signer, key)
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool,shardId uint16) {
	...
	pool := NewTxPool(config, params.TestChainConfig, blockchain,shardId)
	defer pool.Stop()
