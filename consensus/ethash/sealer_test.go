...
func TestRemoteNotify(t *testing.T) {
	...
	case work := <-sink:
		...
		if want := common.BytesToHash(SeedHash(header.NumberU64())).Hex(); work[1] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	for i := 0; i < cap(sink); i++ {
		header := new(types.Header)
		header.FillBy(&types.HeaderStruct{Number: big.NewInt(int64(i)), Difficulty: big.NewInt(100)})
		block := types.NewBlockWithHeader(header)
...
func TestStaleSubmission(t *testing.T) {
	...
	results := make(chan types.BlockIntf, 16)
	...
	for id, c := range testcases {
		...
		header := new(types.Header)
		header.FillBy(c.headers[c.submitIndex])
		if res := api.SubmitWork(fakeNonce, ethash.SealHash(header), fakeDigest); res != c.submitRes {
		...
		case res := <-results:
			...
			if res.Header().Number().Uint64() != c.headers[c.submitIndex].Number.Uint64() {
				t.Errorf("case %d block number mismatch, want %d, get %d", id+1, c.headers[c.submitIndex].Number.Uint64(), res.Header().NumberU64())
			}
