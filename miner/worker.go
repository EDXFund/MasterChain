...
func newWorker(config *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(types.BlockIntf) bool) *worker {
	...
	worker.chainShardSub = eth.BlockChain().SubscribeChainShardsEvent(worker.chainShardCh)
...
// 检查是否应该重新产生一个封包动作，一般是因为交易有变化了：有新的交易收到了，或者是因为新块收到了，需要删除其中的一些交易
// EDX更新，对于主链节点，是否有新的masterBlock到达了，或者是有新的shardInfo到达了后也会产生
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case head := <-w.chainHeadCh: //区块头到达
			clearPending(head.Block.NumberU64())
			timestamp = time.Now().Unix()
			commit(false, commitInterruptNewHead)

		case <-timer.C: //定时器到达，检查是否有新的交易了，如果有，就重新进行打包任务，否则跳过
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case /*ev :=*/ <-w.chainShardCh:  //side chain can be erase in the future
...
func (w *worker) commitUncle(env *environment, uncle types.HeaderIntf) error {
	hash := uncle.Hash()
...
func (w *worker) commitShardTransactions(txs []*types.Transaction, coinbase common.Address, interrupt *int32) bool {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if daoBlock := w.config.DAOForkBlock; daoBlock != nil {
		...
		if header.Number().Cmp(daoBlock) >= 0 && header.Number().Cmp(limit) < 0 {
			...
			if w.config.DAOForkSupport {
				header.SetExtra (common.CopyBytes(params.DAOForkBlockExtra))
			} else if bytes.Equal(header.Extra(), params.DAOForkBlockExtra) {
				header.SetExtra ([]byte{}) // If miner opposes, don't let it use the reserved extra-data
			}
		}
	}
	...
	if !noempty {
		...
		// execution finished.
		w.commit([]types.HeaderIntf{}, nil, false, tstart)
	}

	if w.chain.ShardId() != types.ShardMaster {
		w.buildTransactions(err, interrupt, tstart)
	}else {
		w.buildShardBlockInfos(err, interrupt, tstart)
	}

}
func (w *worker) buildShardBlockInfos(err error, interrupt *int32, tstart time.Time) {
	// Fill the block with all available pending transactions.
	pending, err := w.eth.ShardPool().Pending()
	if err != nil {
	...
	for _,shardBlock := range pending {
		for _,blockInfo := range shardBlock {
			block := rawdb.ReadBlock(w.eth.ChainDb(), blockInfo.Hash(),blockInfo.NumberU64())
			txs := block.Transactions()
			if w.commitShardTransactions(txs, w.coinbase, interrupt) {
				return
			}
		}
	}
...
func (w *worker) buildTransactions(err error, interrupt *int32, tstart time.Time) {
	// Fill the block with all available pending transactions.
...
func (w *worker) commit(uncles []types.HeaderIntf, interval func(), update bool, start time.Time) error {
